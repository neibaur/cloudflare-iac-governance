...
def test_sync_to_google_sheets_overwrites_latest_and_appends_history(mocker, capsys):
    ...
    history_worksheet.row_values.return_value = ["audit_date", "domain_name", "is_compliant"]
    history_worksheet.col_count = 20
...
def test_history_append_extends_an_older_header_and_aligns_rows_by_name(mocker, capsys):
    worksheet = mocker.Mock(name="history_worksheet")
    worksheet.col_values.return_value = ["audit_date", "20260430T010000Z"]
    worksheet.row_values.return_value = ["audit_date", "domain_name", "ssl_mode", "is_compliant"]
    ...
    appended = aggregate_to_sheets.append_new_history_rows(worksheet, dataframe)

    assert appended == 1
    worksheet.add_cols.assert_called_once_with(1)
    worksheet.update.assert_called_once_with(
        [["audit_date", "domain_name", "ssl_mode", "is_compliant", "min_tls_version"]]
    )
    worksheet.append_rows.assert_called_once_with(
        [["20260430T020000Z", "Domain 01", "full", 1, "1.2"]]
    )
    assert "added column(s) to header: min_tls_version" in capsys.readouterr().out


def test_history_append_leaves_columns_missing_from_the_report_blank(mocker):
    worksheet = mocker.Mock(name="history_worksheet")
    worksheet.col_values.return_value = ["audit_date", "20260430T010000Z"]
    ...
    dataframe = pd.DataFrame(
        [{"audit_date": "20260430T020000Z", "domain_name": "Domain 01", "is_compliant": 0}]
    )

    aggregate_to_sheets.append_new_history_rows(worksheet, dataframe)

    worksheet.update.assert_not_called()
    worksheet.append_rows.assert_called_once_with([["20260430T020000Z", "Domain 01", "", 0]])
